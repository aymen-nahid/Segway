...
static i2cState_TypeDef i2cCtrlStatus = NEVER_USED;
...
/**
  ...
  * @retval None
  */
void MeSN_I2C_Init(void){
	
	/* Init I2C bus if needed */
	if (i2cCtrlStatus == NEVER_USED){	/* I2C soft object must be created */
		...
		i2cCtrlStatus = NOT_READY;
	}
	
	if (i2cCtrlStatus == NOT_READY){	/*I2C bus must be configured*/
		/* Init I2C peripheral */
		MX_I2C_Init();
	...
	if (i2cCtrlStatus == READY){	/*I2C bus already configured*/
		//Nothing to do
	}
}
